...
- [日次文字数バジェット（src/daily_budget.py）](#日次文字数バジェットsrcdaily_budgetpy)
- [レポート生成（src/report_generator.py）](#レポート生成srcreport_generatorpy)
...
| 機能 | 概要 |
|---|---|
| RSSフィード取得 | 指定した海外フィードを定期取得。ETag/Last-Modifiedで差分取得 |
| 日本語翻訳 | タイトル・概要を複数エンジン（Google / OpenAI / DeepL / Claude）で翻訳。Google Translate使用時は1日15,000文字の上限を管理 |
| LLM要約 | 自然な日本語タイトルと3行要約をLLMで生成（オプション） |
| 翻訳キャッシュ | 翻訳済み記事をキャッシュし再翻訳を防止。上限10,000件で自動プルーニング |
| RSS生成・公開 | フィードごとのRSS XMLを生成しGitHub Pagesで公開 |
| 翻訳識別プレフィックス | タイトルに `[翻訳]` を付与してフィードリーダーで識別可能にする |
| 翻訳・要約レビューレポート | 翻訳・要約の精度・必要性を確認できる HTML レポートを GitHub Pages に公開 |
| 自動更新 | GitHub Actionsで1日2回（JST 9:00・12:00）自動実行 |

### 成功の定義

- Feedlyで日本語フィードとして他のフィードと並んで違和感なく読める
- 英語の障壁なく海外ITニュースを毎日継続して読める習慣が維持できる

### ビジネス要件

- 追加のサーバーコスト不要（GitHub ActionsとGitHub Pagesの無料枠で運用）
- 翻訳APIのコストを最小限に抑える（キャッシュ・バッチ翻訳で再利用）
- メンテナンスコストを最小化（設定変更だけでエンジン・フィードを切り替え可能）
...
- 手動実行で任意のタイミングでも更新可能
- 翻訳コストを最小化する（キャッシュ・バッチ翻訳・チャンク分割・Google Translate無料枠活用）
- 翻訳APIの1日あたりの利用文字数に上限を設け、コストを予測可能にする

### パフォーマンス要件

- 翻訳APIの呼び出し回数を最小化する（バッチ翻訳・10件チャンク・キャッシュ活用）
- GitHub Actionsの実行時間を現実的な範囲に収める（通常数十秒〜数分以内）
- キャッシュのルックアップはO(1)（GUIDをキーとするJSONオブジェクト）
- キャッシュサイズは最大10,000件に自動プルーニングし、肥大化を防ぐ
...
単一の`main.py`エントリポイントから6つのモジュールを7ステップでシーケンシャルに呼び出す。
...
│   ├── test_models.py
│   ├── test_pipeline.py
│   └── test_daily_budget.py
├── cache/
│   ├── translated.json              # 翻訳済み記事キャッシュ（gitコミット対象）
│   ├── http_cache.json              # ETag/Last-Modifiedキャッシュ（gitコミット対象）
│   └── char_usage.json             # 日次文字数バジェット（gitコミット対象）
├── docs/
│   ├── feed/
│   │   ├── hacker-news.xml          # フィード別RSS（GitHub Pages公開）
│   │   └── dev-community.xml
│   └── report.html                  # 翻訳・要約レビューレポート（GitHub Pages公開）
├── .github/
│   ├── workflows/
...
| 種別 | 配置先 | 備考 |
|---|---|---|
| アプリケーションコード | `src/` | モジュール単位でファイルを分割 |
| 翻訳エンジン実装 | `src/translators/<engine>.py` | エンジン名をファイル名に使用 |
| 要約エンジン実装 | `src/summarizers/<engine>.py` | エンジン名をファイル名に使用 |
| テスト | `tests/test_<module>.py` | 対応するモジュール名をプレフィックスに使用 |
| 翻訳キャッシュ | `cache/translated.json` | gitコミット対象 |
| HTTPキャッシュ | `cache/http_cache.json` | gitコミット対象 |
| 日次バジェット | `cache/char_usage.json` | gitコミット対象。1日2回の実行をまたいでカウントを継続 |
| 生成RSSフィード | `docs/feed/<feed-name>.xml` | GitHub Pages公開対象 |
| レビューレポート | `docs/report.html` | GitHub Pages公開対象。update-rssワークフローと同時に更新 |
| 設計ドキュメント | `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` | |
| 実装計画 | `docs/superpowers/plans/YYYY-MM-DD-<feature>.md` | |
| GitHub Actionsワークフロー | `.github/workflows/<name>.yml` | |
...
translator:
  ...
  # daily_char_limit: 15000  # Google Translate使用時のみ有効。省略時は無制限
...
report:
  output_path: docs/report.html   # レビューレポートの出力先
  items_per_feed: 10              # フィードごとの表示件数
```
...
- **フィード別出力**: `FeedConfig.output_path` に指定されたパスへフィードごとに個別ファイルを生成（`docs/feed/hacker-news.xml` など）
...
### 日次文字数バジェット（src/daily_budget.py）

Google Translate の1日あたりの翻訳文字数を `cache/char_usage.json` で管理するモジュール。1日2回の実行をまたいでカウントを継続し、日付が変わった時点で自動リセットする。
...
日付が現在日と異なる場合は `used` を0にリセットして更新する。

**バジェット超過時の動作（`main.py`）:**

1. 翻訳対象テキストの合計文字数を計算する
2. `can_translate()` がFalseの記事はキャッシュに保存しない（翌日自動で再翻訳される）
3. RSS生成には `translated_title=None` の `TranslatedArticle` として渡す（`generator.py` が `original_title` にフォールバック）
4. `WARNING` ログを出力してスキップ数を記録する
...
### レポート生成（src/report_generator.py）

`main.py` のパイプライン最後に呼び出され、キャッシュ全件から HTML レビューレポートを生成する。翻訳・要約の精度と必要性を目視で確認するためのファイルで、GitHub Pages に公開される。
...
`main.py` は全キャッシュから読み込んだ `list[TranslatedArticle]` を渡す。失敗時は `WARNING` ログを出力してパイプラインを継続する（RSS 生成を止めない）。

#### レポート構造

フィードごとに1セクション、各セクション最新 `items_per_feed`（デフォルト10）件のカードを表示する。

```
docs/report.html
├── ヘッダー（生成日時・合計件数）
├── セクション: Hacker News（最新10件）
│   └── カード × 10
└── セクション: DEV Community（最新10件）
    └── カード × 10
```
...
| フィールド | 表示 |
|---|---|
| 原文タイトル（英語）| `original_title` |
| 翻訳タイトル（日本語）| `translated_title`。None の場合は「翻訳スキップ」バッジを表示 |
| 原文説明（英語）| `original_description`。`<details>` タグで折りたたみ表示 |
| 翻訳済み説明（日本語）| `translated_description`。None の場合は「翻訳なし」と表示 |
| 要約（日本語）| `summary`。None の場合は「要約なし」と表示 |
| 原文リンク | `link` へのアンカーリンク |

#### HTML 仕様

- 外部 CSS フレームワーク・CDN 不使用（`<style>` タグにインライン記述）
- JavaScript 不使用
- 文字コード: UTF-8
- レスポンシブデザイン不要（PC ブラウザでの閲覧を想定）

#### エラーハンドリング

| ケース | 挙動 |
|---|---|
| `translated_title` が None | 原文タイトルのみ表示、「翻訳スキップ」バッジを付与 |
| `summary` が None | 「要約なし」と表示 |
| `translated_description` が None | 「翻訳なし」と表示 |
| フィードに記事が 0 件 | そのフィードのセクションに「記事なし」と表示 |
| `articles` が空リスト | レポート生成をスキップ（ファイルを更新しない）|
| ファイル書き込み失敗 | `WARNING` ログを出力してパイプラインを継続 |
...
| 日次文字数バジェット超過 | `WARNING`ログを出力してスキップ。キャッシュ保存なし（翌日自動再翻訳）。RSS生成には英語のまま含める |
| レポート生成失敗 | `WARNING`ログを出力してパイプラインを継続（RSS生成は止めない）|
...
- **コミット対象**: `cache/`（翻訳キャッシュ）、`docs/feed/`（フィード別 RSS ファイル）、`docs/report.html`（レビューレポート）
...
https://ryu1.github.io/rss-feed-translator/report.html
```
...
命名規則の詳細は [`docs/glossary.md`](../../glossary.md) を参照。

**Git規約**: コミットメッセージは [Conventional Commits](https://www.conventionalcommits.org/) に従う。

```
feat:     新機能
fix:      バグ修正
docs:     ドキュメントのみの変更
chore:    ビルド・自動化・依存関係の更新
