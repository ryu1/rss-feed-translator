...
**目次について：** セクション数が10を超える場合は、ドキュメント先頭に目次を追加する。複数ファイルへの分割はしない（サブエージェントへの受け渡しが複雑になるため）。目次のセクションが多い場合は、関連するセクションを**太字のグループ見出し**でまとめて可読性を高める（例：`**アーキテクチャ・設計**`、`**コンポーネント**`、`**運用・インフラ**`、`**開発**`）。
